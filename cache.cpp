...
public:
    ...
    double read_double(uint64_t addr) {
        ...
        if (!hit) {
            stats_.misses++;
            // Miss de lectura: BusRd (write-allocate en futuras escrituras)
            BusMessage m{BusCmd::BusRd, addr, pe_id_};
            stats_.bus_msgs++;
            SnoopSummary sum = ic_->broadcast(m, this);

            // Traer bloque desde memoria (si otro tenía M ya escribió back)
            auto& vict = choose_victim(set_idx);
            evict_if_dirty(set_idx, vict, addr);
            fill_from_mem(addr, set_idx, vict);

            MESI new_state = sum.shared_seen ? MESI::Shared : MESI::Exclusive;
            record_transition(set_idx, vict, sets_[set_idx][vict].state, new_state, f.tag, addr);
            sets_[set_idx][vict].state = new_state;
            sets_[set_idx][vict].tag = f.tag;
            mark_recent(set_idx, vict);
            way = vict;
        } else {
    ...
    void write_double(uint64_t addr, double value) {
        ...
        if (hit) {
            // Hit: si S → necesito BusUpgr para invalidar otros; E/M escribo local
            if (sets_[set_idx][way].state == MESI::Shared) {
                BusMessage m{BusCmd::BusUpgr, addr, pe_id_};
                stats_.bus_msgs++;
                ic_->broadcast(m, this);
                // S -> M
                record_transition(set_idx, way, MESI::Shared, MESI::Modified, f.tag, addr);
                sets_[set_idx][way].state = MESI::Modified;
            } else if (sets_[set_idx][way].state == MESI::Exclusive) {
        ...
        // Miss de escritura: Write-allocate -> BusRdX, traer, poner en M, escribir
        stats_.misses++;
        BusMessage m{BusCmd::BusRdX, addr, pe_id_};
        stats_.bus_msgs++;
        ic_->broadcast(m, this);

        auto& vict = choose_victim(f.index);
        evict_if_dirty(f.index, vict, addr);
        fill_from_mem(addr, f.index, vict);
        record_transition(f.index, vict, sets_[f.index][vict].state, MESI::Modified, f.tag, addr);
        sets_[f.index][vict].state = MESI::Modified;
        sets_[f.index][vict].tag = f.tag;
        store_into_line(f.index, vict, f.offset, value);
        mark_recent(f.index, vict);
    }
...
SnoopSummary Interconnect::broadcast(const BusMessage& msg, Cache* origin) {
    ...
    for (auto* c : caches_) {
        ...
        sum.mod_seen    = sum.mod_seen    || resp.wrote_back; // aproximación
    }
    return sum;
}
